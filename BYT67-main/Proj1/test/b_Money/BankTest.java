...
public class BankTest {
	...
	public void testOpenAccount() throws AccountExistsException, AccountDoesNotExistException {
		SweBank.openAccount("NewGuy");
		try {
			SweBank.openAccount("Ulrika");
		}catch (AccountExistsException ignored){
			return;
		}
		fail("Shouldn't allow to open accounts with same id");  // failed
	}

	@Test
	public void testDeposit() throws AccountDoesNotExistException {
		SweBank.deposit("Ulrika", new Money(100000, SEK)); // failed
		assertEquals(new Integer(100000), SweBank.getBalance("Ulrika"));
		try {
	...
	public void testWithdraw() throws AccountDoesNotExistException {
		...
		assertEquals(new Integer(90000), SweBank.getBalance("Ulrika")); // failed
		try {
	...
	public void testTransfer() throws AccountDoesNotExistException {
		...
		assertEquals(new Integer(300000), SweBank.getBalance("Bob"));
		assertEquals(new Integer(200000), Nordea.getBalance("Bob"));
		SweBank.transfer("Bob", "Ulrika", new Money(200000, SEK));
		assertEquals(new Integer(100000), SweBank.getBalance("Bob")); // failed
		assertEquals(new Integer(200000), SweBank.getBalance("Ulrika"));
		try{
	...
	public void testTimedPayment() throws AccountDoesNotExistException {
		...
		for(int timer = 0; timer < 15; ++timer){
			SweBank.tick();
			if(timer==3){
				assertEquals(new Integer(460000), SweBank.getBalance("Bob")); // failed
				assertEquals(new Integer(540000), SweBank.getBalance("Ulrika"));
				SweBank.removeTimedPayment("Bob", "first");
				SweBank.addTimedPayment("Ulrika", "second", 2, 3, new Money(30000, SEK), SweBank, "Bob");
			}
		}
		assertEquals(new Integer(550000), SweBank.getBalance("Bob"));
		assertEquals(new Integer(450000), SweBank.getBalance("Ulrika"));
	}
}
