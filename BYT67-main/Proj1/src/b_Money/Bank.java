...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			// bug #1 openAccound method didn't throw 'AccountExistsException' when I've opened 2 accounts with the same id
...
//		 bug #2 deposit method trowed 'AccountDoesNotExistException' when method was invoked using id of an existing account
//		if (accountlist.containsKey(accountid)) {
		if (!accountlist.containsKey(accountid)) {
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			// bug #3 instead of decreasing the amount of money withdraw method has added it to the account
//			account.deposit(money);
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		// bug #4 the amount of money of the sender didn't change
